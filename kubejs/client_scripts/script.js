...
onEvent('item.tooltip', tooltip => {
    let holds = (id, slots) => tooltip.add("iron_barrel:" + id + "_barrel", [`§7${slots} Slots`])
    ...
    not_consumed('bluepower:iron_saw')
    not_consumed('bluepower:diamond_saw')
    not_consumed('bluepower:ruby_saw')
    not_consumed('bluepower:sapphire_saw')
    not_consumed('bluepower:green_sapphire_saw')
    not_consumed('bluepower:amethyst_saw')
    not_consumed('bluepower:tungcarb_saw')
    not_consumed('bluepower:screwdriver')
    ...
    tooltip.add("kubejs:accelerator_redstone", ["§7When used in Alchemy Research:", "  §6One of the §ecorrect §6Reagents",
        "  §6in §eincorrect §6slots will not be consumed"
    ]);
    tooltip.add("kubejs:accelerator_glowstone", ["§7When used in Alchemy Research:", "  §6One of the §ecorrect §6Reagents",
        "  §6in §ecorrect §6slots will not be consumed"
    ]);

    for (i = 0; i < 15; i++)
        tooltip.add(`kubejs:failed_alchemy_${i}`, [
            `§7Place in Centrifugal Separator to analyze.`,
            "",
            "§6Yields",
    ...
    onEvent('jei.information', event => {
        ...
        let beer = (id, igs) =>
            event.add('alcocraft:spruce_mug_empty' + id, ["Place An Empty Beer Mug, " + igs + " in a Keg to create this Beverage."])

        beer("_sun_pale_ale", "1 Hop, 1 Dry Wheat Seed, 1 Sunflower, and 1 Glistering Melon Slice")
        beer("_digger_bitter", "1 Hop, 1 Dry Wheat Seed, 1 Glow Lichen, and 1 Diamond")
        beer("_nether_porter", "1 Hop, 1 Dry Wheat Seed, 1 Gold Nugget, and 1 Blaze Powder")
        beer("_wither_stout", "1 Hop, 1 Dry Wheat Seed, 1 Nether Wart, and 1 Wither Skeleton Skull")
        beer("_magnet_pilsner", "1 Hop, 1 Dry Wheat Seed, 1 Iron Nugget, and 1 Redstone")
        beer("_drowned_ale", "1 Hop, 1 Dry Wheat Seed, 1 Prismarine Crystal, and 1 Prismarine Shard")
        beer("_night_rauch", "1 Hop, 1 Dry Wheat Seed, 1 Charcoal, and 1 Phantom Membrane")
        beer("_ice_beer", "1 Hop, 1 Dry Wheat Seed, 1 Snowball, and 1 Packed Ice")
        beer("_kvass", "1 Hop, 1 Apple, 1 Honeycomb, and 1 Bread")
        beer("_leprechaun_cider", "1 Hop, 1 Dry Wheat Seed, 1 Emerald, and 1 Rabbit's Foot")
        beer("_chorus_ale", "1 Hop, 1 Dry Wheat Seed, 1 Phantom Membrane, and 1 Chorus Fruit")
        beer("_nether_star_lager", "1 Hop, 1 Dry Wheat Seed, 1 Diamond, and 1 Nether Star")
