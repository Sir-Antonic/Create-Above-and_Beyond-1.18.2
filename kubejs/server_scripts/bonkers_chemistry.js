...
function process(world, block, entity, face) {
    ...
    items.forEach(e => {
        ...
        if (e.id.startsWith("kubejs:accelerator_redstone")) {
            redstoneAccelerator = true
            return
        }
        if (e.id.startsWith("kubejs:accelerator_glowstone")) {
            glowstoneAccelerator = true
            return
        }
        if (!e.id.startsWith('kubejs:substrate_')) {
            valid = false
    ...
    if (glowstoneAccelerator || redstoneAccelerator) {
        let random = new java("java.util.Random")()
        let shuffled = shuffle(Array(0, 1, 2, 3), random)
        for (i = 0; i < 4; i++) {
            let j = shuffled[i]
            if (glowstoneAccelerator && resultEval[j] == 2) {
                retain = j
                break
            }
            if (redstoneAccelerator && resultEval[j] == 1) {
                retain = j
                break
            }
        }
        ...
        if (parts) {
            parts.forEach(part => {
                ...
                if (part.side != face.getOpposite().ordinal())
                    return
                valid = true
                color = part.id.replace("_inverted", "").replace("cagelamp", "").replace("bluepower:", "")
            })
        }
